...
from convs.vit import vit_base_patch16_224
...
class IncrementalNet(BaseNet):
    ...
    def forward(self, x, glowflag=False):
        if not glowflag:
            x = self.convnet(x)
        else:
            x = self.convnet(x, glowflag, self.fc.weight.shape[0])
        out = self.fc(x['features'])
        out.update(x)
        if hasattr(self, 'gradcam') and self.gradcam:
...
from models.loss_utils.arploss import ARPLoss, Prompt



class OODIncNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        fc = ARPLoss(0.1, 1.0, self.feature_dim, nb_classes)
        self.prompt_pool.append(Prompt((32,32)))
        self.classifier.append(fc)
        self.classifier_id.append(nb_classes)

    def getPrompt(self, id):
        return self.prompt_pool[id].prompt


    def interface(self, x):
        finallogit = []
        for i, p in enumerate(self.prompt_pool):
            xx = self.convnet(x+p.prompt.expand(x.shape))['features']
            l = self.classifier[i]
            # for j, l in enumerate(self.classifier):
            gcp, _ = l.gcp_logit(xx, None, None)
            arp, _ = l.arp_logit(xx, None, None)
            finallogit.append([gcp, arp])

        index = []
        for i, p in enumerate(self.prompt_pool):
            index.append(finallogit[i][0].mean(1))
        finalindex = torch.stack(index, 0).max(0)[1]
        # pred = [finallogit[finalindex[i]][0][i].max(0)[1].item()+sum(self.classifier_id[:finalindex[i]]) for i in range(len(finalindex))]

        logits = torch.ones(x.shape[0], 100) * -999
        for i in range(len(finalindex)):
            logits[i, sum(self.classifier_id[:finalindex[i]]):sum(self.classifier_id[:finalindex[i]+1])] = finallogit[finalindex[i]][0][i]
    ...
    def fc(self, feature):
        ...
        # 把输入给每一个classifier，并且把loss返回
        for i, l in enumerate(self.classifier):
            gcp, gcploss = l.gcp_logit(feature, None, None)
            arp, arploss = l.arp_logit(feature, None, None)
            gcp_list.append(gcp)
            gcploss_list.append(gcploss)
            arp_list.append(arp)
            arploss_list.append(arploss)

        return {'logits': gcp_list,
                'gcp_logits': torch.cat(gcp_list, 1),
                'arp_logits': torch.cat(arp_list, 1),
                }
